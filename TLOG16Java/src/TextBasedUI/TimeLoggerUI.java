...
ask for end time (format: 12:45, with validation)*/
    ...
    public void startTask(TimeLogger timeLogger) {
        try {
            int workMonth = this.listDays(timeLogger);
            
            if (workMonth == -1 || timeLogger.getMonths().get(workMonth).getDays().isEmpty()) {
                return;
            }
            ...
            while (1 == 1) {
                ...
                if (dayNumber > 0 && dayNumber <= timeLogger.getMonths().get(workMonth).getDays().size()) {
                    break;
                }
                System.out.println("Wrong day numbers!");
            }
            ...
            WorkDay day = timeLogger.getMonths().get(workMonth).getDays().get(dayNumber);
            ...
            timeLogger.updateMonth(timeLogger.getMonths().get(workMonth));
